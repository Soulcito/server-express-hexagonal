...
export default class UserInfraestructure implements UserRepository {
   ...
   async update(guid: string, user: Partial<UserUpdate>): Promise<Result<User, UserNotFoundException | UserEmailInvalidException>> {
      ...
      if ( userFound ) {
         ...
         const emailResult = EmailVO.create(userFound.email);
